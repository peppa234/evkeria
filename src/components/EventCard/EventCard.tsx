...
export function EventCard({ event }: EventCardProps) {
  ...
  const typeLabel = event.badge || event.category || '';
  ...
  const handleFavToggle = async () => {
    ...
    try {
      if (isFav) {
        // Remove from saved
        const res = await fetch(`/api/users/${user.id}/saved-events?eventId=${event.id}`, {
          method: "DELETE",
          credentials: "include",
        });
        if (res.ok) {
  ...
  return (
    <Card className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 ease-out overflow-hidden border-0 hover:scale-[1.03] hover:-translate-y-2 active:scale-100">
      <CardContent className="p-0">
        <div className="relative h-48 sm:h-56">
          ...
          <button
            ...
            className="absolute top-4 right-4 grid place-items-center w-10 h-10 rounded-full bg-white shadow-md hover:shadow-lg transition-all duration-300 hover:scale-110 active:scale-95 disabled:opacity-50"
      ...
      <CardFooter className="flex items-center justify-start px-4 sm:px-6 pb-4 sm:pb-6">
