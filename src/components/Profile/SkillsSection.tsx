
"use client";

type Props = {
  skills: string[];
  onAddSkill: (skill: string) => void;
  onRemoveSkill: (skill: string) => void;
};

export function SkillsSection({ skills, onAddSkill, onRemoveSkill }: Props) {
  const handleAdd = () => {
    const value = prompt("Enter a new skill");
    if (!value) return;
    onAddSkill(value.trim());
  };

  return (
    <section className="max-w-full sm:max-w-2xl md:max-w-4xl lg:max-w-5xl mx-auto mt-6 sm:mt-8 px-4 sm:px-6">
      <div className="rounded-2xl sm:rounded-3xl bg-white shadow-lg overflow-hidden">
        <div className="flex items-center gap-2 bg-[#E4F1FF] px-4 sm:px-6 md:px-8 py-3 sm:py-4">
          <img src="/skills.png" alt="Skills icon" className="w-4 h-4 sm:w-5 sm:h-5" />
          <h2 className="font-outfit font-semibold text-base sm:text-lg text-[#0B2443]">
            Skills
          </h2>
        </div>

        <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-6">
          <div className="flex flex-wrap gap-2 sm:gap-3 mb-3 sm:mb-4">
            {skills.map((skill) => (
              <button
                key={skill}
                onClick={() => onRemoveSkill(skill)}
                className="flex items-center gap-1 sm:gap-2 rounded-full bg-[#E3ECF8] px-3 sm:px-4 py-1 text-xs sm:text-sm font-outfit text-[#1A2B4C]"
              >
                <span>{skill}</span>
                <span className="text-xs">✕</span>
              </button>
            ))}
          </div>

          <button
            onClick={handleAdd}
            className="font-outfit text-xs sm:text-sm text-[#4A7FD9] hover:underline"
          >
            + Add Skill
          </button>
        </div>
      </div>
    </section>
  );
}
