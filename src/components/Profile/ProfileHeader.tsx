...
export function ProfileHeader({ name, email, bio, avatarUrl, onEditClick }: Props) {
    ...
    <section className="w-full">
      <div className="rounded-2xl sm:rounded-3xl overflow-hidden bg-white shadow-lg">
        <div className="bg-[#7DB7E9] h-24 sm:h-28 md:h-32 relative">
          ...
          <button
            onClick={logout}
            className="absolute top-4 right-4 flex items-center gap-2 px-3 py-2 bg-white/90 hover:bg-white rounded-lg shadow-sm transition-all hover:shadow-md text-sm font-medium text-[#1e3a5f]"
            aria-label="Logout"
          >
            <LogOut size={16} />
          ...
          <div className="absolute -bottom-8 sm:-bottom-10 md:-bottom-12 left-4 sm:left-6 md:left-10 flex items-end gap-3 sm:gap-4">
            <div className="relative">
              <Image
                src={avatarUrl || "/profile_pic.png"}
                alt="Profile picture"
                width={112}
                height={112}
                className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full border-2 sm:border-4 border-white shadow-lg object-cover"
              />
              <button
                onClick={onEditClick}
        ...
        <div className="pt-12 sm:pt-14 md:pt-16 pb-6 sm:pb-8 px-4 sm:px-6 md:px-10 flex flex-col gap-2">
          ...
          <div className="mt-2 sm:mt-3 font-outfit text-xs sm:text-sm leading-relaxed text-[#1A2B4C] max-w-3xl prose prose-sm prose-slate">
            <SafeMarkdown
              components={{
